...
def _copy_ticks_range_retry(broker, start: datetime, end: datetime, *, attempts: int = 5):
    last_error = None
    for attempt in range(1, attempts + 1):
        try:
            rows = broker.api.copy_ticks_range(
                broker.symbol, start, end, broker.api.COPY_TICKS_ALL
            )
        except Exception as exc:
            last_error = ("python_exception", repr(exc))
            rows = None
        if rows is not None:
            if attempt > 1:
                _print(
                    "history_read_recovered",
                    attempt=attempt,
            ...
            return rows
        last_error = _mt5_last_error(broker.api) or last_error
        _print(
            "history_read_retry",
            attempt=attempt,
            requested_start=start.isoformat(),
        ...
        if attempt < attempts:
            try:
                broker.api.symbol_info_tick(broker.symbol)
            except Exception:
                pass
            time.sleep(0.5 * (2 ** (attempt - 1)))

    raise SystemExit(
        "MT5 copy_ticks_range returned no result after "
        f"{attempts} attempts for {start.isoformat()} .. {end.isoformat()}; "
        f"last_error={last_error!r}. Do not rebuild samples until collection succeeds."
    )
...
def main() -> None:
    ...
    parser.add_argument("--history-read-attempts", type=int, default=5, help="bounded retries for transient MT5 history sync failures")
    ...
    if not 1 <= args.history_read_attempts <= 8:
        raise SystemExit("--history-read-attempts must be between 1 and 8")
    ...
    try:
        ...
        preflight_start = raw_end - timedelta(minutes=5)
        preflight = _copy_ticks_range_retry(
            broker, preflight_start, raw_end, attempts=args.history_read_attempts
        )
        _print(
            "history_preflight",
            requested_start=preflight_start.isoformat(),
            requested_end=raw_end.isoformat(),
            rows=len(preflight),
            mt5_last_error=_mt5_last_error(broker.api),
        )
        ...
        while cursor < raw_end:
            ...
            rows = _copy_ticks_range_retry(
                broker, cursor, end, attempts=args.history_read_attempts
            )
            chunks += 1
            received = len(rows)
